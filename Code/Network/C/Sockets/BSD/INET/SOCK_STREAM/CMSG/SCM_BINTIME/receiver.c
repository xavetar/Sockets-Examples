...
int decode_bintime(const struct bintime *timestamp) {
    // Convert time_t to the broken-down time (struct tm)
    struct tm *time_info = localtime(&timestamp->sec);
    ...
    printf("Timestamp (strftime - bintime): %s\n", time_str);

    // Print the time in the manual format
    printf("Timestamp (manually - bintime): %02d-%02d-%04d %02d:%02d:%02d.%06llu\n\n",
           time_info->tm_mday, time_info->tm_mon + 1, time_info->tm_year + 1900,
           time_info->tm_hour, time_info->tm_min, time_info->tm_sec, (unsigned long long) timestamp->frac);
...
int process_cmsg(struct cmsghdr* cmsg) {
    if (cmsg != NULL) {
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_BINTIME) {
            ...
            return decode_bintime(timestamp);
        }
...
int main() {
    ...
    int timestamp_option = SO_TS_BINTIME;
    if (setsockopt(
            socket_file_descriptor, SOL_SOCKET, SO_BINTIME, &timestamp_option, sizeof(timestamp_option)
        ) == -1) {
        perror("setsockopt");
